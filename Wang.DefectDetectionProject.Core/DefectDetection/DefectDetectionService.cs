...
namespace Wang.DefectDetectionProject.Core.DefectDetection
{
    /// <summary>
    /// 缺陷检测服务
    /// </summary>
    public class DefectDetectionService : BindableBase
    {
        public DefectDetectionService()
        {
            ImageListItems = new ObservableCollection<ImageListItem>();
        }

        /// <summary>
        /// 外部提供的HWindow窗口
        /// </summary>
        private HTuple? externalWindowHandle;
        /// <summary>
        /// 外部提供的HWindow窗口
        /// </summary>
        public HTuple? ExternalWindowHandle
        {
            get { return externalWindowHandle; }
            set
            {
                externalWindowHandle = value;
        ...
        /// 图像列表
        /// </summary>
        private ObservableCollection<ImageListItem>? imageListItems;
        /// <summary>
        /// 图像列表
        /// </summary>
        public ObservableCollection<ImageListItem>? ImageListItems
        {
            get { return imageListItems; }
            set
            {
                imageListItems = value; 
                RaisePropertyChanged();
            }
        }

        /// <summary>
        /// 推理时使用的batch size，需≥1
        /// </summary>
        ...
        /// <summary>
        /// 缺陷过滤面积大小（小于这个面积的缺陷忽略）
        /// </summary>
        private int filterationArea = 0;
        /// <summary>
        /// 缺陷过滤面积大小（小于这个面积的缺陷忽略）
        /// </summary>
        public int FiltrationArea
        {
            get { return filterationArea; }
            set
            {
                filterationArea = value;
                if(filterationArea < 0)
                {
                    filterationArea = 0;
                }
                RaisePropertyChanged();
        ...
        HTuple? hv_DLDeviceHandles = null, hv_DLDevice = null, hv_DLModelHandle = null, hv_DLPreprocessParam = null;
        HTuple? hv_ClassNames = null, hv_ClassIDs = null, hv_DLSampleBatch = null, hv_DLResultBatch = null;

        /// <summary>
        /// 运行缺陷检测（可批量，一次推理的 batch_size 可指定）
        /// </summary>
        /// <param name="images">待检测图像集合</param>
        /// <param name="preprocessParamDictFileName">预处理参数字典文件路径</param>
        /// <param name="trainedParamDictFileName">训练好的模型文件路径</param>
        /// <exception cref="HalconException"></exception>
        public async Task Run(ObservableCollection<HObject> images,
                        string preprocessParamDictFileName,
                        string trainedParamDictFileName)
        {
            HOperatorSet.ClearWindow(ExternalWindowHandle);
            foreach(var item in ImageListItems!)
            {
                item.DefectDetectionResults = new ObservableCollection<DefectDetectionResult>();
            }
            await Task.Run(() =>
            {
                // ①.获取硬件设备
                ...
                if ((int)(new HTuple((new HTuple(hv_DLDeviceHandles.TupleLength())).TupleEqual(0))) != 0)
                {
                    throw new HalconException("No supported device found to continue this program.");
                }
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_DLDevice = hv_DLDeviceHandles.TupleSelect(0);
                }

                // ②.读取模型，设置模型相关参数
                HOperatorSet.ReadDlModel(trainedParamDictFileName, out hv_DLModelHandle);
                ...
                // 设置 batch_size（一次设置，后面所有批次共用）
                int finalBatchSize = Math.Max(1, Math.Min(BatchSize, images.Count));
                HOperatorSet.SetDlModelParam(hv_DLModelHandle, "batch_size", new HTuple(finalBatchSize));
                // 设置推理使用的硬件设备
                HOperatorSet.SetDlModelParam(hv_DLModelHandle, "device", hv_DLDevice);

                // ③.读取预处理参数字典
                HOperatorSet.ReadDict(preprocessParamDictFileName, new HTuple(), new HTuple(), out hv_DLPreprocessParam);

                // ④.准备收集全部样本字典与结果字典
                hv_DLSampleBatch = new HTuple();
                hv_DLResultBatch = new HTuple();

                HTuple defectClassIDs = hv_ClassIDs;
                HTuple defectColors = new HTuple(
                    "#00FF0080",  // 纯绿
                    "#FF000080",  // 纯红
                    "#0000FF80",  // 纯蓝
                    "#FFFF0080",  // 纯黄
                ...
                for (int startIdx = 0; startIdx < total; startIdx += finalBatchSize)
                {
                    int count = Math.Min(finalBatchSize, total - startIdx);
                    // 提取当前批次的图像
                    var batchImages = new ObservableCollection<HObject>();
                    for (int j = 0; j < count; j++)
                        batchImages.Add(images[startIdx + j]);

                    // 生成当前批次的样本字典
                    HObject combinedBatch = CombineImagesToObject(batchImages);
                    gen_dl_samples_from_images(combinedBatch, out HTuple subSampleBatch);

                    // 预处理
                    preprocess_dl_samples(subSampleBatch, hv_DLPreprocessParam);

                    // 推理
                    HOperatorSet.ApplyDlModel(
                        hv_DLModelHandle,
                        subSampleBatch,
                        (new HTuple("segmentation_image")).TupleConcat("segmentation_confidence"),
                        out HTuple subResultBatch);

                    // 把子批次字典句柄拼接到总集合中
                    hv_DLSampleBatch = hv_DLSampleBatch.TupleConcat(subSampleBatch);
                    hv_DLResultBatch = hv_DLResultBatch.TupleConcat(subResultBatch);

                    // ⑥.显示结果
                    for (int i = 0; i < count; i++)
                    {
                        HOperatorSet.GetDictObject(out HObject preprocessImg, subSampleBatch.TupleSelect(i), "image");
                        HOperatorSet.GetDictObject(out HObject segmentationImg, subResultBatch.TupleSelect(i), "segmentation_image");

                        HObject imageItem = GenerateOverlayImage(preprocessImg, segmentationImg, defectClassIDs, defectColors, ImageListItems![startIdx + i]);
                        // 更新图像列表的图像为叠加了缺陷区域的图像
                        ImageListItems[startIdx + i].Image = imageItem;
                        ImageListItems[startIdx + i].ImgSource = new HImage(imageItem).ToBitmapSource();
                    }
                }
            });
        }
        ...
        /// <summary>
        /// 功能方法：将ObservableCollection<HObject>集合中记录的图片合并为一个HObject返回
        /// </summary>
        /// <param name="images"></param>
        /// <returns></returns>
        private static HObject CombineImagesToObject(ObservableCollection<HObject> images)
        {
            if (images == null || images.Count == 0)
                return new HObject(); // 返回空对象

            HObject combined = images[0].Clone(); // 从第一张开始（避免修改原图）

            for (int i = 1; i < images.Count; i++)
            {
                HOperatorSet.ConcatObj(combined, images[i], out HObject temp);
                combined.Dispose();   // 释放旧的合并对象
                combined = temp;      // 指向新的合并结果
            }
            return combined;
        }

        /// <summary>
        /// 功能方法：将缺陷区域叠加到预处理后的图像上一并显示
        /// </summary>
        /// <param name="window"></param>
        /// <param name="image"></param>
        /// <param name="segmentationImage"></param>
        /// <param name="classIDs"></param>
        /// <param name="colors"></param>
        private static void DisplaySegmentationOverlay(HWindow window, HObject image, HObject segmentationImage, HTuple classIDs, HTuple colors)
        {
            // 1. 清空原来的显示，显示原始图像
            window.ClearWindow();
            window.DispObj(image);

            // 2. 设置绘制模式为填充（区域内部填充颜色）
            window.SetDraw("fill");

            // 3. 遍历每个类别，提取缺陷区域并叠加显示
            for (int i = 0; i < classIDs.Length; i++)
            {
                ...
                HOperatorSet.Threshold(segmentationImage, out classRegions, classID, classID);
                
                HOperatorSet.Connection(classRegions, out HObject connectedRegions);
                // AreaCenter方法输出的areas是一个数组，表示了这种缺陷的所有区域的面积构成的数组。。。
                HOperatorSet.AreaCenter(connectedRegions, out HTuple areas, out HTuple rows, out HTuple columns);  

                // 检查区域是否非空（避免显示空区域）
                HOperatorSet.RegionFeatures(classRegions, "area", out HTuple area);
                if (area > 0)
                {
                    HOperatorSet.SetColor(window, colors[i]);           // 设置当前类别的颜色（含透明度）
                    window.DispObj(classRegions);                       // 显示该区域
                }
            }
        ...
        /// <summary>
        /// 功能方法：生成缺陷区域叠加到原图上的3通道 byte类型 HObject图像
        /// </summary>
        /// <param name="image"></param>
        /// <param name="segmentationImage"></param>
        /// <param name="classIDs"></param>
        /// <param name="colors"></param>
        /// <param name="imageListItem"></param>
        /// <returns></returns>
        private HObject GenerateOverlayImage(HObject image, HObject segmentationImage,
                                             HTuple classIDs, HTuple colors, ImageListItem imageListItem)
        {
            // 1. 原图转 byte 三通道
            HObject imageByte;
            ...
            {
                //double scale;
                //HOperatorSet.MinMaxGray(image, image, 1.0, out HTuple minVal, out HTuple maxVal, out _);
                //if(maxVal.D - minVal.D == 0.0)
                //    scale = 255.0;
                //else
                //    scale = 255.0 / (maxVal.D - minVal.D);
                //double add = -minVal.D * scale;
                //HOperatorSet.ScaleImage(image, out HObject scaled, scale, add);

                HOperatorSet.ScaleImageMax(image, out HObject scaledMax);
                HOperatorSet.ConvertImageType(scaledMax, out imageByte, "byte");
            }
            else
            {
                HOperatorSet.CopyImage(image, out imageByte);
            }

            HOperatorSet.CountChannels(imageByte, out HTuple channels);
            if (channels != 3)
            {
                HObject ch1, ch2, ch3;
                if (channels == 1)
                {
                    HOperatorSet.CopyImage(imageByte, out ch1);
                    HOperatorSet.CopyImage(imageByte, out ch2);
                    HOperatorSet.CopyImage(imageByte, out ch3);
                }
                else
                {
                    HOperatorSet.AccessChannel(imageByte, out ch1, 1);
                    HOperatorSet.AccessChannel(imageByte, out ch2, 2);
                    HOperatorSet.AccessChannel(imageByte, out ch3, 3);
                }
                HOperatorSet.Compose3(ch1, ch2, ch3, out imageByte);
            }

            HOperatorSet.ConvertImageType(imageByte, out HObject imageReal, "real");
            HOperatorSet.GetImageSize(imageReal, out HTuple width, out HTuple height);

            if (imageListItem.DefectDetectionResults == null)
                imageListItem.DefectDetectionResults = new ObservableCollection<DefectDetectionResult>();
            ...
            for (int i = 0; i < classIDs.Length; i++)
            {
                ...
                HOperatorSet.Threshold(segmentationImage, out classRegions, classID, classID);
                //HOperatorSet.CountObj(classRegions, out HTuple numRegions);

                HOperatorSet.Connection(classRegions, out HObject connectedRegions);

                // 这里可以过滤面积过小的缺陷。。。
                HOperatorSet.SelectShape(connectedRegions, out HObject filteredRegions, "area", "and", FiltrationArea, 9999999);

                // 这里的AreaCenter方法输出的areas是一个数组，表示了这种缺陷的所有区域的面积构成的数组。。。
                HOperatorSet.AreaCenter(filteredRegions, out HTuple areas, out _, out _);

                if (areas.Length == 0) continue;

                HObject unitedRegion;
                HOperatorSet.Union1(filteredRegions, out unitedRegion);
                // 这里的AreaCenter方法输出的area是这种缺陷的总面积大小
                HOperatorSet.AreaCenter(unitedRegion, out HTuple area, out _, out _);

                if (area == 0) continue;

                // 记录该图像的缺陷信息
                imageListItem.DefectDetectionResults.Add(new DefectDetectionResult()
                {
                    FileName = imageListItem.Title,
                    DefectName = hv_ClassNames![i],
                    MarkingColor = colors[i],
                    Area = area,
                    Count = areas.Length,
                    DetectionResult = "NG"
                });

                (int r, int g, int b, int a) = ParseRGBA(colors, i);

                // 生成纯色图层（三通道，彩色）
                HObject colorLayer;
                HOperatorSet.PaintRegion(unitedRegion, imageReal, out colorLayer,
                                         new HTuple(r, g, b), "fill");

                // 生成 Alpha 蒙版（单通道，灰度）
                HObject alphaBase;
                HOperatorSet.GenImageConst(out alphaBase, "real", width, height);
                HObject alphaMask;
                HOperatorSet.PaintRegion(unitedRegion, alphaBase, out alphaMask, a, "fill");

                // 混合
                imageReal = BlendWithAlpha(imageReal, colorLayer, alphaMask);
            }

            if(imageListItem.DefectDetectionResults.Count == 0)
            {
                imageListItem.DefectDetectionResults.Add(new DefectDetectionResult()
                {
                    FileName = imageListItem.Title,
            ...
            HObject resultByte;
            HOperatorSet.ConvertImageType(imageReal, out resultByte, "byte");
            return resultByte;
        }

        /// <summary>
        /// 使用 alpha 蒙版混合前景和背景图像（均为 real 三通道）
        /// </summary>
        private static HObject BlendWithAlpha(HObject background, HObject foreground, HObject alphaMask)
        {
            // alphaMask 是单通道 real 图像，值范围 0-255
            // 归一化为 0~1
            HObject alphaNorm;
            HOperatorSet.ScaleImage(alphaMask, out alphaNorm, 1.0 / 255.0, 0);

            // 分别混合三个通道
            HObject bgR, bgG, bgB, fgR, fgG, fgB;
            HOperatorSet.AccessChannel(background, out bgR, 1);
            HOperatorSet.AccessChannel(background, out bgG, 2);
            HOperatorSet.AccessChannel(background, out bgB, 3);
            HOperatorSet.AccessChannel(foreground, out fgR, 1);
            HOperatorSet.AccessChannel(foreground, out fgG, 2);
            HOperatorSet.AccessChannel(foreground, out fgB, 3);

            HObject newR = MixChannel(bgR, fgR, alphaNorm);
            HObject newG = MixChannel(bgG, fgG, alphaNorm);
            HObject newB = MixChannel(bgB, fgB, alphaNorm);

            HObject result;
            HOperatorSet.Compose3(newR, newG, newB, out result);
            return result;
        }

        private static HObject MixChannel(HObject bg, HObject fg, HObject alpha)
        {
            // result = bg * (1 - alpha) + fg * alpha
            HObject invAlpha, temp1, temp2, result;
            HOperatorSet.ScaleImage(alpha, out invAlpha, -1, 1);     // 1 - alpha
            HOperatorSet.MultImage(bg, invAlpha, out temp1, 1, 0);
            HOperatorSet.MultImage(fg, alpha, out temp2, 1, 0);
            HOperatorSet.AddImage(temp1, temp2, out result, 1, 0);
            return result;
        }

        private static (int r, int g, int b, int a) ParseRGBA(HTuple colors, int index)
        {
            HTuple item = colors[index];
            if (item.Type == HTupleType.STRING)
            {
                string hex = item.S.Replace("#", "").Trim();
                if (hex.Length >= 6)
                {
            ...
            {
                return (item[0].I, item[1].I, item[2].I, item.Length >= 4 ? item[3].I : 255);
            }
            throw new ArgumentException($"Unsupported color format at index {index}");
        }
        ...
        private void check_dl_preprocess_param(HTuple hv_DLPreprocessParam)
        {
            ...
            try
            {
                ...
                if ((int)((new HTuple(hv_Index.TupleEqual(-1))).TupleOr(new HTuple(hv_Index.TupleEqual(
                    new HTuple())))) != 0)
                {
                    throw new HalconException(new HTuple("Only models of type '3d_gripping_point_detection', 'anomaly_detection', 'classification', 'detection', 'gc_anomaly_detection', 'multi_label_classification', 'ocr_recognition', 'ocr_detection' or 'segmentation' are supported"));
                }
                //
                ...
                for (hv_I = 0; (int)hv_I <= (int)((new HTuple(hv_InputKeys.TupleLength())) - 1); hv_I = (int)hv_I + 1)
                {
                    ...
                    {
                        throw new HalconException(("Unknown key for DLPreprocessParam: '" + (hv_InputKeys.TupleSelect(hv_I))) + "'");
                    }
                    ...
                    {
                        if ((int)((new HTuple(hv_DLModelType.TupleEqual("anomaly_detection"))).TupleOr(
                            new HTuple(hv_DLModelType.TupleEqual("gc_anomaly_detection")))) != 0)
                        {
                            hv_ValidValues.Dispose();
                            hv_ValidValues = new HTuple();
                ...
                if ((int)((new HTuple((new HTuple(hv_DLModelType.TupleEqual("classification"))).TupleOr(
                    new HTuple(hv_DLModelType.TupleEqual("multi_label_classification"))))).TupleOr(
        ...
        private void preprocess_dl_model_augmentation_data(HTuple hv_DLSample, HTuple hv_DLPreprocessParam)
        {
            ...
            HTuple hv_DLPreprocessParam)
        {
            ...
            try
            {
                ...
                hv_PreserveDomain.Dispose();
                hv_PreserveDomain = 0;
                if ((int)((new HTuple((new HTuple(hv_ModelType.TupleEqual("3d_gripping_point_detection"))).TupleAnd(
                    (new HTuple(hv_DomainHandling.TupleEqual("crop_domain"))).TupleOr(new HTuple(hv_DomainHandling.TupleEqual(
                    "keep_domain")))))).TupleOr((new HTuple(hv_ModelType.TupleEqual("gc_anomaly_detection"))).TupleAnd(
                ...
                else if ((int)(new HTuple(hv_DomainHandling.TupleEqual("crop_domain"))) != 0)
                {
                    ...
                    (new HTuple((new HTuple(hv_ModelType.TupleEqual("anomaly_detection"))).TupleOr(
                    new HTuple(hv_ModelType.TupleEqual("gc_anomaly_detection"))))).TupleOr(
        ...
        private void preprocess_dl_model_3d_data(HTuple hv_DLSample, HTuple hv_DLPreprocessParam)
        {
            // Stack for temporary objects 
            HObject[] OTemp = new HObject[20];
        ...
        private void preprocess_dl_model_images_ocr_recognition(HObject ho_Images, out HObject ho_ImagesPreprocessed,
            HTuple hv_DLPreprocessParam)
        {
            // Stack for temporary objects 
            HObject[] OTemp = new HObject[20];
        ...
        private void preprocess_dl_model_images_ocr_detection(HObject ho_Images, out HObject ho_ImagesPreprocessed,
            HTuple hv_DLPreprocessParam)
        {
            // Stack for temporary objects 
            HObject[] OTemp = new HObject[20];
